  ...
  useEffect(() => {
    if (data?.managedAccount) {
      setAccount({
        ...
        managedAccountLinks: data.managedAccount.managedAccountLinks?.map(acc => {
          return {
  ...
  return (
    <div className="managed-account-info-section scroll-container">
      ...
      <p className="managed-account-info-section-description">
        Update the details of your managed account here. You can also connect your platform accounts to this account.
      </p>

      <Card className="managed-account-info-card">
        <h3>General</h3>
        <p className="section-description">General details about the managed account</p>

        <ManagedAccountsByIdGeneralInfo account={account}
                                        setAccount={setAccount}
                                        onSave={() => {}} />

        <div className="platform-accounts-section">
          <h3>Platform Accounts</h3>
          <p className="section-description">Connect your platform accounts to this account</p>

          <AccountLinkCard platformName="YouTube"
                           onClickConnect={handleYouTubeOAuth}
                           onClickDisconnect={() => {}}
                           isConnected={account.managedAccountLinks
                             ?.some(link => link.tokenFor === "YOUTUBE")
                             || false}
          />
